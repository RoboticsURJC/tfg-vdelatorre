...
with open("ficherito.txt","r") as file:
    lineas = file.readlines()
    
with open("ficherito.txt","w") as file:    
...
for i in range(140):
    ...
    connect_to_wifi(ap_info[2]['ssid'], ap_info[2]['password'])
    ...
    rssi = get_signal_strength(ap_info[2]['ssid'])
    print("RSSI FINAL: ", rssi)
    #file.write(f"{rssi},")
        
    with open("ficherito.txt","r") as file:
        lineas_act = file.readlines()
        
    lineas_act[i] = lineas_act[i].strip() + f"{rssi},16\n"
    
    with open("ficherito.txt","w") as file:    
        file.writelines(lineas_act)
