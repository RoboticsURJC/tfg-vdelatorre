...
'''
with soundfile.SoundFile("/home/torre/Desktop/WAV/prueba_fold/audio_prueba_delante.wav") as audio:
    happy_waveform = audio.read(dtype="float32")
    sample_rate = audio.samplerate

with soundfile.SoundFile("/home/torre/Desktop/WAV/prueba_fold/audio_prueba_atras.wav") as audio:
...
def load_data():
    X,y=[],[]
    count = 0
    for file in glob.glob("/home/torre/Desktop/WAV/fold_final/*.wav"):
...
#es decir, tiene que haber como minimo mas de una muestra de cada clase ya que sino la covarianza es 0
        print('\r' + f' Processed {count}/{120} audio samples',end=' ')
...
plt.xticks(ticks=range(2), labels = [direction for direction in direction_list],fontsize=10)
...
joblib.dump(model, 'modelo_final_entrenado.pkl')
'''
loaded_model = joblib.load('modelo_mlp_entrenado.pkl')

audio_path_prueba = '/home/torre/Desktop/WAV/prueba_fold/delante_rasp_45-.wav'
...
unique_classes = np.unique(np.concatenate((test_direction_groundtruth, test_direction_predictions)))
direction_name = [direction for direction in unique_classes]
